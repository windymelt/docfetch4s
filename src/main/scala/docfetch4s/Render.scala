...
/** 標準出力に出す文字列の組み立て。
  *
  * 想定する読み手は AI エージェントなので、装飾より構造の一貫性を優先し、
  * 折り返しは行わない。`--json` を選べば同じ内容を機械可読な形で出す。
  */
object Render:

  /** 一覧に載せる 1 行の要約。原文の改行位置で切れて見えないよう空白に潰し、長ければ省略する。
    * コード例は 1 行に収まらないためフェンスの手前で打ち切り、その導入句だけが残らないようにする。
    */
  ...
  def typePage(
    ...
    if entry.signature.nonEmpty && entry.signature != entry.name then
      sb ++= s"  ${entry.signature}\n"
    sb ++= s"  [$coords]\n"
